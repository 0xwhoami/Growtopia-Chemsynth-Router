...
class ChemsynthPoint(Chemsynth):
	...
	def __point_side(self, index, direction, temp):
		...
		if self.dom[index] == self.tar[index]:
			bonus = -1
		if self.dom[index] != self.tar[index] and self.dom[index] == self.dom[index+direction] and self.dom[index+direction] == self.tar[index+direction]:
			bonus += -1
		if temp.dom[index] != self.tar[index] and self.dom[index+direction] == self.tar[index+direction] and temp.dom[index] == self.dom[index+direction]:
			bonus += -1
	...
	def catalyst_point(self, index):
		...
		#calculating point
		bonus = self.__point_side(index, -1, temp)		#left point
		bonus += self.__point_side(index, 1, temp)		#right point
		bonus += self.__general_bonus(old_block, target_block, new_block)
		bonus += self.__point1(new_block, target_block) - self.__point1(old_block, target_block)
	...
	def stirrer_point(self, index):	
		...
		if self.dom[index-1] == self.dom[index+1]:
			return bonus
		bonus = self.__point_side(index-1, -1, temp)		#left point
		bonus += self.__point_side(index+1, 1, temp)		#right point
		bonus += self.__general_bonus(old_block, target_block, new_block)
		bonus += self.__point1(new_block, target_block) - self.__point1(old_block, target_block)
	...
	def centrifuge_point(self, index):
		...
		#out of range mitigation
		if (index == 0) or (index == len(self.dom)-1):			#value // 2 == value >> 1
			return 0
		...
		bonus += self.__point_side(index-max_range, -1, temp)		#left point
		bonus += self.__point_side(index+max_range, 1, temp)		#right point
		bonus += self.__general_bonus(old_block, target_block, new_block)
